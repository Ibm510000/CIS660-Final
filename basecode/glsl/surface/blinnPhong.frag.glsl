...
{
    // TODO Homework 4
    //DO Lambert calculate first
    vec4 diffuseColor=texture2D(u_Texture,fs_UV);
    ...
    float shinessterm=20.f;
    //Do not need to create another light source, just use diffuse Color
    vec3 specularColor=diffuseColor.rgb;
    vec3 specular=pow(dot(HalfDir,normalize(fs_Nor)),shinessterm)*specularColor;
    
    out_Col=vec3(diffuseColor.rgb*lightIntensity)+specular.rgb;
}

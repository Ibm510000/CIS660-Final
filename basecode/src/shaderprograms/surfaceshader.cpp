...
SurfaceShader::SurfaceShader(OpenGLContext *context)
    : ShaderProgram(context),
      ...
      unifModel(-1), unifModelInvTr(-1), unifView(-1), unifProj(-1),uniCamera_pos(-1)
{}
...
void SurfaceShader::setupMemberVars()
{
    ...
    //Bind uniCamera_pos with shader uniform variable
    uniCamera_pos=context->glGetUniformLocation(prog,"unif_Camera");
...
//Pass camera to the uniform variable in the shader
void SurfaceShader::setCameraPosition(const Camera camera)
{
    useMe();
    if(uniCamera_pos!=-1)
    {
        context->glUniform3f(uniCamera_pos,camera.eye.x,camera.eye.y,camera.eye.z);
    }
}
